...
**9. Home Feed (Property mode)** — H(56px): wordmark left · **city chip** (pin icon + "Mavdi, Rajkot" + chevron, tap→city sheet, feed live-refreshes in place) · bell(badge) · messages(badge). Under H: StoryRow (edge-fade, no add-circle) → SegmentedToggle Property|Requirement → chips row (Buy/Rent + sort pill Latest▾). N: Home active (re-tap scroll-top, 2nd refresh). C: 1,2,7,10,11,30,31; cards per Doc1-C1. S: city sheet (search+recent cities), card ⋯ (Share/Report/Not interested), report sheet, inquiry sheet (→ request state; guest→login sheet), native share, PWA install card, iOS guide overlay, banner slot, "New listings" pill, nudge card (own 0-inquiry). The caught-up marker and PULL-TO-REFRESH were both removed 9 Aug 2026 (Rajan) — the screen ends in its FOOTER (home only: wordmark, Explore links, Legal links from cms_pages, copyright), and a refresh is now only the "New listings" pill or a city switch. **Rails (reordered 8 Aug 2026 — Rajan):** HomzList top picks → Newly-added properties → Featured Developers → Featured Brokers → Featured properties (everything boosted) → Have a property to sell? (CTA → /create) → News and Articles (→ /blog). Per-type rails and the suggested strip were removed in the same change; each rail keeps its heading "View all" (a pill) and scrolls endlessly, with no end-of-rail tile, and a rail holding a single card lets it take the full column. St: guest (actions walled), cascade-labeled sections, new-city empty (+nearby auto — **6 Aug 2026, widened again 9 Aug 2026**: a city with zero live listings AND zero live projects now falls back to ALL INDIA, not to its state, so there is never a blank feed; the rail subtitles drop the place name entirely and a notice above them says "No listings in <City> yet — be the first to list your property here. Meanwhile, showing properties from all over India" with a List-your-property button). The **guest's city chip scopes the server**, not just its own label: the picked city rides every feed read as `?city=`, validated against `locations`; a signed-in profile's city always wins. F: entry from splash/nav/deep homzlist.com; scroll restore on back; bell→61 msgs→34; notif-landing: various per map; deep: / → here. R: **Builder = dashboard feed** (own stat cards: views/leads/units + matched RequirementCards locked/unlocked, cascade label once per GROUP not per card; no others' listings); guest no badges.
...
**10. Home Feed (Requirement mode)** — Same shell; RequirementCards (C3/C4) locked/unlocked; boosted top (locked-but-top unpaid); chips: Buy/Rent/type. S: paywall sheet, proposal compose (2-option), + Mode-9 sheets. St: unpaid locked-grid + persistent unlock banner; **city-empty → "Other cities in <State>" section; no city known (guest who never picked one) → "Across India" section; nothing anywhere → server empty copy + "Change city" opening the shell's city sheet**. The GUEST's city chip scopes this mode too (it is sent to the server), not just Property mode. F: toggle from 9; state persists per device.
...
**40. Requirements Browse** — H: "Requirements" + my-icon(→41) + filter. Body: cards locked/unlocked; boosted top; cascade sections. N: center active. S: paywall, proposal compose, filter, **city sheet** (reached from the empty state's "Change city"; selecting PATCHes the profile city, same write as the feed's city chip). St: unpaid locked-grid + unlock banner; **empty = server-supplied copy + a working "Change city" CTA** (city empty → state fallback section first; nothing anywhere → the empty state). Builder with no live project: the card's action is "Post a Project" + the reason line, never a Send Proposal that 403s. F: nav → 19; back→9.
